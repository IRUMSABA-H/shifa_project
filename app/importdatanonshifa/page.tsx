...
import Addformnonshifa from"../patient/nonshifa/Addformnonshifa"; // 👈 Path check kar lein sahi hai na?
import type { NonShifaFormData, NonShifaFormRef } from "../patient/nonshifa/Addformnonshifa";
import { useRouter } from "next/navigation";

export default function NewModalPage() {
  // 1. Ref banaya jo nonShifaForm ke andar access kary ga
  const formRef = useRef<NonShifaFormRef>(null);
  ...
  useEffect(() => {
    ...
    if (savedData && formRef.current) {
      try {
        ...
        // 4. Kaam hone ke baad temporary storage saaf kar di
  ...
  const handleFinalSave = (data: NonShifaFormData) => {
    console.log("Final Submitted Data:", data);
    alert("Data imported and saved successfully!");
    router.push("/nonshifa"); // Wapis main table par bhej do 
  };
